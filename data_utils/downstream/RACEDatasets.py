    ...
    def process_data(self):
        ...
        for d in lines[:int(self.ratio * len(lines))]:
            ...
            if self.prompt_config:
                ...
                enc_input_ids = [-(i + 1) for i in range(prompt_len)] + context_ids + [117] + question_ids + [58] + choice_ids + [58] + self.tokenizer.encode("The correct one is ") + [extra_id_0]
            else:
                lim = 512 - len(question_ids) - len(choice_ids) - 10
                context_ids = context_ids[:lim]
                enc_input_ids = context_ids + self.tokenizer.encode("Based on the previous passage, we ask ") + question_ids + [58] + choice_ids + [58] + self.tokenizer.encode("The correct one is ") + [extra_id_0]

            target = [0, extra_id_0] + [number_map[d["answer"]]]
