    ...
    def process_data(self):
        ...
        for idx, d in df.iloc[:til].iterrows():
            ...
            if self.prompt_config:
                ...
                context = [-(i + 1) for i in range(prompt_len)] + sid + [extra_id_0]
            else:
                context = sid + self.tokenizer.encode("Before that PersonX needs ") + [extra_id_0] + [5]

            target = [0, extra_id_0] + self.tokenizer.encode(self.label_map_fn(d["target_text"]))
    ...
    def process_data(self):
        ...
        for line in lines[:int(self.ratio * len(lines))]:
            ...
            if self.prompt_config:
                ...
                prompt_len = self.prompt_config["enc"]["prompt_len"]
                context = [-(i + 1) for i in range(prompt_len)] + sid + [58] + choice_ids + [58] + self.tokenizer.encode("The correct one is ") + [extra_id_0]
            else:
                context = sid + self.tokenizer.encode("It was ") + [extra_id_0] + [5]

            target = [0, extra_id_0] + [self.label_map[d["label"]]]
