    ...
    def process_data(self):
        ...
        for line in lines[:int(self.ratio * len(lines))]:
            ...
            if self.prompt_config:
                ...
                context = [-(i + 1) for i in range(prompt_len)] + self.tokenizer.encode(d["hypothesis"]) + [extra_id_0] + self.tokenizer.encode(d["premise"])
            else:
                context =  self.tokenizer.encode(d["hypothesis"] + "?") + [extra_id_0] + self.tokenizer.encode(".") + self.tokenizer.encode(d["premise"])
