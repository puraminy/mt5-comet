...
class LMPretrainDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        contexts = contexts[:511] + [extra_id_0]

        # contexts = [-(i + 1) for i in range(int(self.enc_prompt_len))] + contexts

        targets = [int(x) for x in targets]
        targets = [1, extra_id_0] + targets[2:]

        labels = targets[1:]
        targets = targets[:-1]

        contexts = contexts + [self.tokenizer.pad_id] * \
            (self.enc_seq_length - len(contexts))
        targets = targets + [self.tokenizer.pad_id] * \
            (self.dec_seq_length - len(targets))
        labels = labels + [self.tokenizer.pad_id] * \
            (self.dec_seq_length - len(labels))

        assert len(contexts) <= self.enc_seq_length, (len(
            contexts), self.enc_seq_length)
        assert len(targets) <= self.dec_seq_length, (len(
            targets), self.dec_seq_length)
