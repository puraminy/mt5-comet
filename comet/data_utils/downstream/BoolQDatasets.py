    ...
    def process_data(self):
        ...
        for line in lines[:int(self.ratio * len(lines))]:
            ...
            if self.prompt_config:
                ...
                context = [-(i + 1) for i in range(prompt_len)] + qid + [self.extra_id_0] + pid
            else:
                qid = self.tokenizer.encode(d["question"])
                max_passage_len = 512 - len(qid) - len(self.tokenizer.encode("question: ")) - len(self.tokenizer.encode(" passage: "))
                pid = self.tokenizer.encode(d["passage"])[:max_passage_len]
                context = self.tokenizer.encode("question: ") + qid + [58] + [self.extra_id_0] + [5] + pid

            target = [0, self.extra_id_0] + self.tokenizer.encode("yes" if d["label"] else "no")
    ...
    def process_data(self):
        ...
        for line in lines[:int(self.ratio * len(lines))]:
            ...
            if self.prompt_config:
                ...
                max_passage_len = 512 - prompt_len - 1 - len(qid)
                pid = self.tokenizer.encode(d["passage"])[:max_passage_len]
                context = [-(i + 1) for i in range(prompt_len)] + qid + [58] + pid + [5] + choice_ids + [58] + self.tokenizer.encode("The correct one is ") + [self.extra_id_0]
            else:
                pass

            target = [0, self.extra_id_0] + [279 if d["label"] else 188]
