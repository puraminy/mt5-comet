...
runlite run -cpu -exp $path -bc base -ov $g1 -var model_id=t5-small--method=unsup-wrap-nat#unsup#unsup-wrap-nat--rel_filter=xIntent--train_samples=50--epochs_num=2--repeat=4--temp_num=5--loop=True--test_samples=100 --follow_method=True --scorers="rouge-bert" --data_path=data/atomic2020 --do_valid=False --val_samples=10 --encoder_type=lstml --cycle=100 $g2 --seed=123 --batch_size=8 --merge_prompts=lstm 
